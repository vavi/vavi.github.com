...
# 实践篇 
1. 线程应该设置线程名称,可以通过java.util.concurrent.ThreadFactory来修改默认的线程名称
2. 获取多线程运行结果: 1.while true中获取调用结果 2.使用回调  3.使用future task

# 实现篇
## 简单锁示例,避免虚假唤醒,使用自旋锁,使等待的线程处于wait状态,等待被notify,而不是处于等待获得锁状态,非重入锁.
...
## 可重入锁示例,如果当前调用线程已经对该Lock实例加了锁，那么while循环就不会被执行.如果当前锁已经被加锁过多次。在unlock()调用没有达到对应lock()调用的次数之前，锁不会被解除
